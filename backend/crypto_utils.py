...
def check_header(decrypted_data) -> bool:
    prefix = decrypted_data[:8]
    mess_time = int.from_bytes(decrypted_data[8:16], byteorder='little')
    # check prefix
    if (prefix != b'ESPHERA|'):
    ...
    print(mess_time)
    print(curr_time)
    return curr_time - mess_time < 300


def decrypt_check_header(encrypted_data, base64_key: str) -> bool:
...
def decrypt_debug(encrypted_data, base64_key: str):
    ...
    try:
        ...
        if len(decrypted_bytes) > 8:
            ...
            rest = decrypted_bytes[16:]
            print(f"\nPrefiks: {prefix}")
            print(f"Reszta (jako liczba int?): {int.from_bytes(time, byteorder='little')}") # lub 'big'
            return prefix, int.from_bytes(time, byteorder='little'), rest

    except Exception as e:
        print(f"BŁĄD: {e}")


def check_header1(encrypted_data, base64_key: str) -> bool:
